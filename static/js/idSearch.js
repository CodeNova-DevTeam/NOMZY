...
// 5. 인증번호 받기 버튼 클릭 로컬 스토리지
// authenticationNumberButton.addEventListener("click", function (event) {
...
//     const foundUser = users.find(user => user.name === name && user.tel === tel);

//     if (foundUser) {
//         generatedAuthCode = "123456"; // 임시로 인증번호 123456
//         isAuthNumberSent = true; // 인증번호 발송 상태로 변경
//         isAuthNumberVerified = false; // 인증번호가 아직 확인되지 않음
//         validationStates.authCode = false; // 인증번호 재발송 시 상태 초기화

//         alert(`인증번호 [${generatedAuthCode}]이(가) 발송되었습니다. (개발용: 실제로는 SMS 발송)`);
...
//     } else {
//         alert("입력하신 이름과 전화번호로 가입된 정보가 없습니다.");
//         authenticationNumberError.textContent = "가입된 정보가 없습니다.";
...
// 최종 아이디 찾기 폼 제출 로컬 스토리지
// 7. 최종 아이디 찾기 폼 제출 이벤트 (로컬 스토리지 버전)
...
//         authenticationNumberError.style.display = "block";
//         isValidForm = false;
//         authenticationNumberButton.focus(); // 버튼에 포커스
//     } else if (!isAuthNumberVerified) {
//         authenticationNumberError.textContent = "인증번호 확인을 완료해주세요.";
//         authenticationNumberError.style.display = "block";
//         isValidForm = false;
//         authenticationNumber.focus(); // 인증번호 입력 필드에 포커스
//     }


//     if (isValidForm) {
//         // 로컬스트로지 에서 아이디(이메일) 찾기
//         const users = JSON.parse(localStorage.getItem("users")) || [];
//         const name = searchName.value.trim();
//         const tel = searchTel.value.trim();
//         const enteredAuthCode = authenticationNumber.value.trim();

//         // 이름과 전화번호, 그리고 인증번호가 일치하는 사용자를 찾음
//         const foundUser = users.find(user =>
//             user.name === name && user.tel === tel && enteredAuthCode === generatedAuthCode
//         );

//         if (foundUser) {
//             alert(`회원님의 아이디는 ${foundUser.email} 입니다.`);
//             console.log("아이디 찾기 성공:", foundUser.email);
//             // 성공 후 로그인 페이지 등으로 이동하거나 폼 초기화
//             // window.location.href = '/templates/login.html'; 나중에 메인url 걸어주기
//             idSearchForm.reset(); // 폼 필드 초기화
//             searchNameError.style.display = "none";
//             searchTelError.style.display = "none";
//             authenticationNumberError.style.display = "none";
//             generatedAuthCode = ""; // 인증번호 초기화
//             isAuthNumberSent = false;
//             isAuthNumberVerified = false;
