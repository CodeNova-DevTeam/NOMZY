...
//         searchNameError.style.display = "block";
//         userSearchName.focus(); // 오류 필드에 포커스
//         return;
//     }
...
//         searchTelError.style.display = "block";
//         userSearchTel.focus(); // 오류 필드에 포커스
//         return;
//     }
...
//         // 임시인증번호 123456
//         generatedAuthCode = "123456"; 
//         isAuthNumberSent = true;
//         isAuthNumberVerified = false;

//         alert(`인증번호 [${generatedAuthCode}]이(가) 발송되었습니다. (테스트용 실제로는 SMS 발송)`);
...
//  최종 비밀번호 재설정 폼 제출 이벤트 (로컬 스토리지 버전)
...
//         authenticationNumberError.style.display = "block";
//         isValidForm = false;
//         userSearchName.focus(); // 이름 필드에 포커스
//     }

//     if (isValidForm) {
//         // Local Storage에서 사용자 정보 업데이트 (비밀번호 변경)
//         let users = JSON.parse(localStorage.getItem("users")) || [];
//         const name = userSearchName.value.trim();
//         const tel = userSearchTel.value.trim();

//         // 이름과 전화번호가 일치하는 사용자를 찾음 (인증번호는 이미 검증되었으니 굳이 다시 찾을 필요는 없음)
//         const foundUserIndex = users.findIndex(user => 
//             user.name === name && user.tel === tel
//         );

//         if (foundUserIndex !== -1) {
//             users[foundUserIndex].password = newPasswordInput.value.trim();
//             localStorage.setItem("users", JSON.stringify(users));

//             alert("비밀번호가 성공적으로 재설정되었습니다. 새 비밀번호로 로그인해주세요.");
//             console.log("비밀번호 재설정 성공:", users[foundUserIndex]);
...
//             return true;
//         } else {
//             alert("입력하신 정보와 일치하는 사용자를 찾을 수 없거나 인증에 실패했습니다.");
//             return false;
//         }
//     } else {
