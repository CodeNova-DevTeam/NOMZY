...
window.check = async function (event) { // onsubmit="return check(event)" 형태로 호출되도록 'event' 객체를 받을수있게함
    ...
    const email = userIdInput.value.trim();
    const password = userPasswordInput.value.trim();
    const serverLoginUrl = "http://100.74.28.37:8081/api/login"; // 여기( # )에 빽엔드 로그인 url 넣기
        ...
        if (response.ok) {
          if (data.isLoggedIn) {
            ...
            localStorage.setItem("nickname",data.nickname);
              ...
              console.log("res")
              console.log("로그인한 사용자 정보:", data.nickname);
              ...
              console.log("실패")
              alert(`로그인패실패: ${data.message || '아이디 또는 비밀번호를 다시 확인해주세요.'}`);
...
document.addEventListener('DOMContentLoaded', () => {
    // 모든 에러 메시지 초기화 및 숨김
    const errorMessages = document.querySelectorAll('.error-message-class'); // 에러 메시지 span에 특정 클래스를 부여했다면 활용
    if (errorMessages.length === 0) { // 만약 특정 클래스가 없다면, id를 직접 선택
